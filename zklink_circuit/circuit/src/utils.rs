...
pub fn multiply_based_on_u126<E: Engine, CS: ConstraintSystem<E>>(
    ...
    pre_check_to_prevent_overflow(cs.namespace(|| "Pre-check to prevent overflow"), a, b)?;

    let product = a
        .get_number()
        .mul(cs.namespace(|| "product"), b.get_number())?;
    ...
    let reduction_factor =
        AllocatedNum::alloc_constant(cs.namespace(|| "reduction_factor"), || {
    ...
    let quotient = CircuitElement::from_fe_with_known_length(
        ...
        || {
            if let (Some(a), Some(b)) = (
                product.get_number().get_value(),
                reduction_factor.get_value(),
            ) {
    ...
    let quotient_mul_b = quotient
        .get_number()
        .mul(cs.namespace(|| "quotient * b"), &reduction_factor)?;
    let upper_bound =
        quotient_mul_b.add(cs.namespace(|| "(quotient + 1) * b"), &reduction_factor)?;
    ...
    E::Fr: FeConvert,
{
    ...
    let disallow_divisor_is_zero =
        AllocatedNum::alloc_constant(cs.namespace(|| "disallow divisor is zero"), || {
    ...
    let amplification_factor =
        AllocatedNum::alloc_constant(cs.namespace(|| "amplification_factor"), || {
            Ok(E::Fr::from_u64(10u64.pow(precision as u32)))
        })?;
    // a <= 126 bits , amplification_factor <= 10^18, so no overflow.
    let magnify_a = a
        .get_number()
        .mul(cs.namespace(|| "magnify_a"), &amplification_factor)?;
    ...
    let quotient_mul_b = quotient
        .get_number()
        .mul(cs.namespace(|| "quotient * b"), &divisor)?;
    ...
    E::Fr: FeConvert,
{
    ...
    pre_check_to_prevent_overflow(cs.namespace(|| "Pre-check to prevent overflow"), x, y)?;
    ...
    let k = CircuitElement::from_fe_with_known_length(
        ...
        || {
            let x_big_uint = x
                .get_number()
                .get_value()
                .map_or(BigUint::one(), FeConvert::into_big_uint);
            let y_big_uint = y
                .get_number()
                .get_value()
                .map_or(BigUint::one(), FeConvert::into_big_uint);
    ...
    let product = x
        .get_number()
        .mul(cs.namespace(|| "x_temp mul y_temp"), y.get_number())?;
    let product = CircuitElement::from_number_with_known_length(
        cs.namespace(|| "x * y"),
        product,
        E::Fr::CAPACITY as usize,
    ...
    let k_add_one = k
        .get_number()
        .add_constant(cs.namespace(|| "k + 1"), E::Fr::one())?;
