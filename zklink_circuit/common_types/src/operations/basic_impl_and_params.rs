...
// op
impl DepositOp{ pub const OP_CODE: u8 = Deposit::TX_TYPE; pub const CHUNKS: usize = 3; }
impl TransferToNewOp{ pub const OP_CODE: u8 = 0x02; pub const CHUNKS: usize = 3; }
impl WithdrawOp{ pub const OP_CODE: u8 = Withdraw::TX_TYPE; pub const CHUNKS: usize = 3; }
impl TransferOp{ pub const OP_CODE: u8 = Transfer::TX_TYPE; pub const CHUNKS: usize = 2; }
impl FullExitOp{ pub const OP_CODE: u8 = FullExit::TX_TYPE; pub const CHUNKS: usize = 3; }
impl ChangePubKeyOp{ pub const OP_CODE: u8 = ChangePubKey::TX_TYPE; pub const CHUNKS: usize = 3; }
impl ForcedExitOp{ pub const OP_CODE: u8 = ForcedExit::TX_TYPE; pub const CHUNKS: usize = 3; }
impl OrderMatchingOp{ pub const OP_CODE: u8 = OrderMatching::TX_TYPE; pub const CHUNKS: usize = 4; }
...
impl From<NoopOp> for ZkLinkOp {
    fn from(op: NoopOp) -> Self { Self::Noop(op) }
}

impl From<DepositOp> for ZkLinkOp {
    fn from(op: DepositOp) -> Self { Self::Deposit(Box::new(op)) }
}

impl From<WithdrawOp> for ZkLinkOp {
    fn from(op: WithdrawOp) -> Self { Self::Withdraw(Box::new(op)) }
}

impl From<TransferOp> for ZkLinkOp {
    fn from(op: TransferOp) -> Self { Self::Transfer(Box::new(op)) }
}

impl From<TransferToNewOp> for ZkLinkOp {
    fn from(op: TransferToNewOp) -> Self { Self::TransferToNew(Box::new(op)) }
}

impl From<FullExitOp> for ZkLinkOp {
    fn from(op: FullExitOp) -> Self { Self::FullExit(Box::new(op)) }
}

impl From<ChangePubKeyOp> for ZkLinkOp {
    fn from(op: ChangePubKeyOp) -> Self { Self::ChangePubKeyOffchain(Box::new(op)) }
}

impl From<ForcedExitOp> for ZkLinkOp {
    fn from(op: ForcedExitOp) -> Self { Self::ForcedExit(Box::new(op)) }
}

impl From<OrderMatchingOp> for ZkLinkOp {
    fn from(op: OrderMatchingOp) -> Self { Self::OrderMatching(Box::new(op)) }
}
