use crate::{helpers::{pack_fee_amount, unpack_fee_amount}, Withdraw, ZkLinkAddress};
use crate::{AccountId, Nonce, TokenId};
use anyhow::{ensure, format_err};
use num::{BigUint, ToPrimitive};
use serde::{Deserialize, Serialize};
use zklink_basic_types::{ChainId, SubAccountId};
use zklink_crypto::params::{ACCOUNT_ID_BIT_WIDTH, BALANCE_BIT_WIDTH, CHAIN_ID_BIT_WIDTH, CHUNK_BYTES, GLOBAL_ASSET_ACCOUNT_ID, TOKEN_BIT_WIDTH, SUB_ACCOUNT_ID_BIT_WIDTH, FEE_BIT_WIDTH, ETH_ADDRESS_BIT_WIDTH, NONCE_BIT_WIDTH};
...
impl GetPublicData for WithdrawOp{
    fn get_public_data(&self) -> Vec<u8> {
        ...
        data.extend_from_slice(if self.tx.fast_withdraw == 1 { self.tx.nonce } else { Nonce(0) }.to_be_bytes().as_ref());// 4
    ...
    pub fn from_public_data(bytes: &[u8]) -> Result<Self, anyhow::Error> {
       ...
       let sub_account_id = bytes[sub_account_offset];
        ...
        let fee = unpack_fee_amount(
            &bytes[fee_offset..to_address_offset],
        ...
        ensure!(is_required, "Source token or target token is mismatching in Withdraw Pubdata");
