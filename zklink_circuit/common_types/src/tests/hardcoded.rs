...
use crate::{
    ...
    tx::{ChangePubKey, ForcedExit, PackedEthSignature, TimeRange, Transfer, WrappedWithdraw, WrappedTransfer, Withdraw},
    Log, PriorityOp,
};
use lazy_static::lazy_static;
use zklink_basic_types::{AccountId, Address, Nonce, TokenId, H256, ChainId};

#[cfg(test)]
pub mod operations_test {
    use crate::{Deposit, FullExit, Withdraw};
    use super::*;
    use crate::tx::{ChangePubKeyECDSAData, ChangePubKeyEthAuthData};
    use crate::operations::GetPublicData;

    // Public data parameters, using them we can restore `ZkLinkOp`.
    ...
    fn test_public_data_conversions_withdraw() {
        let expected_op = {
            ...
            WithdrawOp { tx, account_id, l1_target_token_after_mapping: Default::default() }
        };
...
pub mod tx_conversion_test {
    ...
    lazy_static! {
        ...
        static ref BOB: Address =
            Address::from_str("21abaed8712072e918632259780e587698ef58da").unwrap();
        static ref WITHDRAWP_CHAIN_ID: ChainId = ChainId(1);
