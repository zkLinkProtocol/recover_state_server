...
use zklink_types::tx::{PackedEthSignature, TxSignature};

fn bench_signature_musig_verify(b: &mut Bencher<'_>) {
...
fn bench_signature_seckp_recover(b: &mut Bencher<'_>) {
    ...
    b.iter_batched(
        setup,
        |(secp, msg, sign)| {
            let _ = black_box(secp.recover_ecdsa(&msg, &sign));
        },
        BatchSize::SmallInput,
    );
}
...
pub fn bench_signatures(c: &mut Criterion) {
    ...
    group.bench_function(
        "bench_signature_verify_musig",
        bench_signature_musig_verify,
    );
    group.bench_function(
