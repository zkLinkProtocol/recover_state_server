...
pub fn rescue_hash_tx_msg(msg: &[u8]) -> Vec<u8> {
    assert!(params::PAD_MSG_BEFORE_HASH_BITS_LEN >= msg.len());
...
pub fn rescue_hash_order(msg: &[u8]) -> Vec<u8> {
...
mod test {
    use super::*;
    use crate::franklin_crypto::bellman::{pairing::ff::BitIterator, Field};
    ...
    fn test_get_bits() {
        ...
        let bits = Fr::zero().get_bits_le_fixed(512);
