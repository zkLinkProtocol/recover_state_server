...
impl RecoverProgress {
    pub async fn new(config: &RecoverStateConfig) -> Self{
        ...
        let total_verified_block = zklink_contract.get_total_verified_blocks().await.unwrap().into();
        Self{
    ...
    pub(crate) async fn get_progress(&self) -> Progress{
        ...
        Progress{ current_block, total_verified_block: self.total_verified_block }
    }
}
