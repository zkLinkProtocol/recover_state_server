...
use zklink_crypto::params::{MAIN_SUB_ACCOUNT_ID, FEE_DENOMINATOR, precision_magnified};
use zklink_types::{AccountUpdates, Order, OrderMatching, operations::{OrderMatchingOp, OrderContext}, AccountUpdate};
...
impl TxHandler<OrderMatching> for ZkLinkState {
    ...
    fn create_op(&self, tx: OrderMatching) -> Result<Self::Op, anyhow::Error> {
        ...
        let actual_fee_token = Self::get_actual_token_by_sub_account(tx.sub_account_id,tx.fee_token);
        ensure!(
...
impl ZkLinkState {
    ...
    fn calculate_actual_exchanged_amounts(op: &OrderMatchingOp) -> Option<(BigUint, BigUint)>{
        ...
        if op.tx.maker.is_sell.is_one() {
            // When maker sell base token, taker obtain base token and maker obtain quote token
            (actual_quantity.clone(), actual_exchanged.clone())
        } else {
            // When maker buy base token, taker obtain quote token and maker obtain base token
            (actual_exchanged.clone(), actual_quantity.clone())
        }.into()
    }

    fn verify_order_accounts(&self, tx: &OrderMatching, check_signature: bool) -> Result<(OrderContext, OrderContext), Error> {
        let verify_account = |order: &Order, price: &BigUint| {
            ...
            let slot = account.get_order(Self::get_actual_slot(order.sub_account_id, order.slot_id));

            // Check order nonce
            let residue = if !slot.residue.is_zero(){
                ensure!(order.nonce == slot.nonce || order.nonce == slot.nonce + 1, "Order nonce does not match");
