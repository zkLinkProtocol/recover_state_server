...
impl TxHandler<Transfer> for ZkLinkState {
    ...
    fn create_op(&self, tx: Transfer) -> Result<Self::Op, anyhow::Error> {
        ...
        let outcome = if let Some((to, _)) = self.get_account_by_address(&tx.to) {
            let transfer_op = TransferOp { from: tx.account_id, tx,  to };

            TransferOutcome::Transfer(transfer_op)
        } else {
            let to = self.get_free_account_id();
            let transfer_to_new_op = TransferToNewOp { from: tx.account_id, tx,  to };
...
impl ZkLinkState {
    pub fn apply_transfer_op(
        ...
        let from_real_token = Self::get_actual_token_by_sub_account(op.tx.from_sub_account_id, op.tx.token);
        let to_real_token = Self::get_actual_token_by_sub_account(op.tx.to_sub_account_id, op.tx.token);
        ...
        *from_account.nonce += 1;
        ...
        updates.push((
            ...
            AccountUpdate::UpdateBalance {
                balance_update: (op.tx.token, op.tx.from_sub_account_id, from_old_balance, from_new_balance),
        ...
        updates.push((
            op.to,
            AccountUpdate::UpdateBalance {
                balance_update: (op.tx.token, op.tx.to_sub_account_id, to_old_balance, to_new_balance),
    ...
    fn apply_transfer_op_to_self(
        ...
        ensure!(old_balance >= &op.tx.amount + &op.tx.fee, "Transfer account is not enough balance");
        ...
        *account.nonce += 1;
        ...
        updates.push((
            op.from,
            AccountUpdate::UpdateBalance {
                balance_update: (op.tx.token, op.tx.from_sub_account_id, old_balance, new_balance),
                old_nonce,
                new_nonce,
            },
        ));
        ...
        updates.push((
            op.to,
            AccountUpdate::UpdateBalance {
                balance_update: (op.tx.token, op.tx.to_sub_account_id, old_balance, new_balance),
                old_nonce,
                new_nonce,
            },
        ));
