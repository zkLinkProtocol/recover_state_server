...
use zklink_types::{
    Account, AccountUpdate, AccountUpdates, Deposit, DepositOp, Nonce, SubAccountId,
};
...
impl TxHandler<Deposit> for ZkLinkState {
    ...
    fn create_op(&self, tx: Deposit) -> Result<Self::Op, anyhow::Error> {
        ...
        assert!(
            is_required,
            "Source token or target token is mismatching in creating DepositOp"
        );

        let account_id = self
            .get_account_by_address(&tx.to)
            .map_or_else(|| self.get_free_account_id(), |(account_id, _)| account_id);
    ...
    fn apply_op(&mut self, op: &mut Self::Op) -> Result<AccountUpdates, anyhow::Error> {
        ...
        {
            let real_token =
                Self::get_actual_token_by_sub_account(op.tx.sub_account_id, op.tx.l2_target_token);
...

        {
            ...
            let real_token = Self::get_actual_token_by_chain(
                op.tx.from_chain_id,
                op.l1_source_token_after_mapping,
            );
            ...
            updates.push((
                ...
                AccountUpdate::UpdateBalance {
                    balance_update: (
                        op.l1_source_token_after_mapping,
                        SubAccountId(*op.tx.from_chain_id),
                        global_old_amount,
                        global_new_amount,
                    ),
                    old_nonce: Nonce(0),
